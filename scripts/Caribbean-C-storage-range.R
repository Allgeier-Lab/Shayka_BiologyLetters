...
df_range <- purrr::map_dfr(list(ag = c(agb_mean, agb_sd), bg = c(bgb_mean, bgb_sd)), function(i) {
  ...
  # calculate sum of all cells for low mean and high
  total_lower <- sum(sample(x = samples[samples < lower], size = n_cells, replace = TRUE))
  
  total_mean = sum(sample(x = samples, size = n_cells, replace = TRUE))

  total_higher <- sum(sample(x =  samples[samples > upper], size = n_cells, replace = TRUE))
  
  data.frame(lower = total_lower, mean = total_mean, higher = total_higher)
  
}, .id = "part")

# tidyr::pivot_longer(df_range, -part, names_to = "range") |> 
ggplot(df_range, aes(x = factor(part))) + 
  geom_errorbar(aes(ymin = lower, ymax = higher), linewidth = 1, width = 0.25) + 
  geom_point(aes(y = mean), shape = 19, size = 5) +
  theme_classic()
